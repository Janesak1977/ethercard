...
static boolean FINSended = false; 
...
static byte www_client_internal_result_cb(byte fd, byte statuscode, word datapos, word len_of_data) {
  if (fd!=www_fd)
    (*client_browser_cb)(4,0,0);
  	...
  	if (statuscode==0 && len_of_data>12 && client_browser_cb)
	  {
...
word EtherCard::accept(const word port, word plen) {
  ...
  if (gPB[TCP_DST_PORT_H_P] == (port >> 8) && gPB[TCP_DST_PORT_L_P] == ((byte) port))	//is DSTPort==port
  {
    if (gPB[TCP_FLAGS_P] & TCP_FLAGS_SYN_V)        //have SYN flag set?
      make_tcp_synack_from_syn();
    else
		if (gPB[TCP_FLAGS_P] & TCP_FLAGS_ACK_V)  	//have ACK flag set?
		{
      		...
      		if (info_data_len > 0)
			{
				...
				if (gPB[TCP_FLAGS_P] & TCP_FLAGS_FIN_V) 	//have FIN flag set?
...
word EtherCard::packetLoop (word plen)
{
  ...
  {  	
	...
	{
      tcp_client_state = 2;     	//status SendedSYN (2)
...
//      Serial.write("S");
  ...
  if (eth_type_is_arp_and_my_ip(plen))  	//is EthernetType==ARP and IP==myIP?
  {
    if (gPB[ETH_ARP_OPCODE_L_P]==ETH_ARP_OPCODE_REQ_L_V)
        make_arp_answer_from_request();
    if (waitgwmac & WGW_ACCEPT_ARP_REPLY && (gPB[ETH_ARP_OPCODE_L_P]==ETH_ARP_OPCODE_REPLY_L_V) && client_store_gw_mac())
      waitgwmac = WGW_HAVE_GW_MAC;    		//We have Gateway MAC address
    return 0;
  }
  
  if (eth_type_is_ip_and_my_ip(plen)==0)   	//is EthernetType==ARP and IP==myIP?
  {
    return 0;
  }
  ...
  //******** check if received DSTPort==our SRCPort (generated) ***********  
  if ( gPB[TCP_DST_PORT_H_P]==TCPCLIENT_SRC_PORT_H)
  {
    if (check_ip_message_is_from(hisip)==0)   	//is message from server defined in websrvip?  
    {
      return 0;
    }
    if (gPB[TCP_FLAGS_P] & TCP_FLAGS_RST_V)		//is RST flag set?
	{
      if (client_tcp_result_cb)
        (*client_tcp_result_cb)((gPB[TCP_DST_PORT_L_P]>>5)&0x7,3,0,0);
	  tcp_client_state = 5;        		// status ResetConnection (5)
      return 0;
    }
    
    len = get_tcp_data_len();
    if (tcp_client_state==2)  		//have status==SendedSYN(2)?
	{
      if ((gPB[TCP_FLAGS_P] & TCP_FLAGS_SYN_V) && (gPB[TCP_FLAGS_P] & TCP_FLAGS_ACK_V))     //Have SYN and ACK flag set?
	  {
        make_tcp_ack_from_any(0,0);     //send packet with ACK set
//        Serial.write("A");
        gPB[TCP_FLAGS_P] = TCP_FLAGS_ACK_V|TCP_FLAGS_PUSH_V;  	//Set ACK and PUSH flag
        if (client_tcp_datafill_cb)
          len = (*client_tcp_datafill_cb)((gPB[TCP_SRC_PORT_L_P]>>5)&0x7);
        else
          len = 0;
        tcp_client_state = 3;    	//status SendedDATA (3)
        make_tcp_ack_with_data_noflags(len);	//send packet with ACK,PUSH flags set and DATA payload
//		Serial.write("D");	
      }
	  else
	  {
        ...
        if (gPB[TCP_FLAGS_P] & TCP_FLAGS_ACK_V) 	//Have ACK flag set?
          len = 0;
        make_tcp_ack_from_any(len,TCP_FLAGS_RST_V);  //send RST flag set
//        Serial.write("R");
      }
      return 0;
    }
    
    if (tcp_client_state==3 && len>0)  	//have status==SendedDATA(3) && len>0
	{ 
	  if (client_tcp_result_cb)
	  {
		...
		{
            make_tcp_ack_from_any(len,TCP_FLAGS_PUSH_V);  //send packet with ACK and PUSH flags set
        }
        else
		{
		    make_tcp_ack_from_any(len,TCP_FLAGS_PUSH_V|TCP_FLAGS_FIN_V); //send ACK,PUSH and FIN flags set
            tcp_client_state = 6; 	//state SendedFIN (6)
            FINSended = true;
 //           Serial.write("F");
        }
        return 0;
    ...
    if (tcp_client_state != 5)    		//Only if we don't received ResetConnection(5)
	{
      if (gPB[TCP_FLAGS_P] & TCP_FLAGS_FIN_V)  		//Have FIN flag set?
	  {
		...
		if (FINSended==false)
		{
        	make_tcp_ack_from_any(len+1,TCP_FLAGS_PUSH_V|TCP_FLAGS_FIN_V);  	//set PUSH and FIN flags
        }
        
        tcp_client_state = 6; // state SendedFIN&PUSH
 //       Serial.write("E");
        if (FINSended==false)
        	TcpEnded = true;
        FINSended = false;
